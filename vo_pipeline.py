...
class VOConfig:
    ...
    use_gt_scale: bool = True      # always True for monocular w/ GT
    min_baseline: float = 0.1      # skip frame if t < 0.1 (normalised units)
    verbose: bool = False
...
class MonocularVO:
    ...
    for img, _, gt_pose in dataset:
        result = vo.process_frame(img, gt_pose)
    traj = vo.trajectory
    """
    ...
    def __init__(self, K: np.ndarray, cfg: VOConfig = VOConfig()):
        ...
        # Current world pose (R_cw, t_cw  →  cam-in-world: t = camera origin)
        self._R = np.eye(3)
    ...
    def process_frame(
        self,
        img: np.ndarray,
        gt_pose: Optional[np.ndarray] = None,   # (4,4) world-to-cam or (3,) position
    ) -> dict:
        """
        ...
        gt_pose  : if (4,4) → treated as T_world2cam; if (3,) → camera origin in world
        ...
        if gt_pose is not None:
            if np.array(gt_pose).shape == (4, 4):
                gt_pos = np.array(gt_pose)[:3, 3]
            else:
                gt_pos = np.array(gt_pose).ravel()[:3]
        ...
        # ── Track ────────────────────────────────────────────────────────
        pts_prev_ok, pts_curr_ok, track_mask = self.tracker.track(
        ...
        # ── Compose poses ────────────────────────────────────────────────
        # Convention: _R, _t = R_cam2world, t_cam_origin_in_world
        # Relative motion is in camera frame of previous step:
        #   new_origin = R_prev_c2w @ (scale * t_rel) + t_prev
        #   new_R_c2w  = R_prev_c2w @ R_rel.T        (R_rel is R_curr2prev)
